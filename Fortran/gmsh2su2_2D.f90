...
!
!========================================================================
! ver 0.06 Fixed rectangle element output
!          Fixed MARKER_FAR output
!========================================================================
!========================================================================
...
101 format('NDIME= ',i1)
   ...
   do ie=1,n_elements
     ...
     !
     !write (*,*) element_type(ielement_type)
     ...
     bmarkNew=tags(1)
     if( 2 <= ielement_type .and. ielement_type <= 3 )then
       if (icel==0) then
     ...
     1 format(i8,8(1x,i8),2(1x,i4))
       select case(ielement_type)
         ...
         case(2) ! 3-node tri
           ...
           write(IOcel,*) 5, v(1),v(2),v(3),icel-1
           i3 = i3 + 1
           
         case(3) ! 4-node quad

           write(IOcel,*) 9,v(1),v(2),v(3),v(4),icel-1
           i4q = i4q + 1
...
91 format('NPOIN= ',i10)
   ...
   do iloop=1,n_nodes
     write(IOcel,'(3g16.9,6x,i9)') (node(iloop,i),i=1,2),iloop-1
   enddo 
   !------------------------------------------------------------
   ! write out markers
   !------------------------------------------------------------
...
111 format('NMARK= ',i10)
   do j=1,n_names-1
       ...
       do i=1,tbnd(j)
            if  (tv0(i,j)==3) then
                write(IOcel,*) tv0(i,j),tv1(i,j),tv2(i,j)
            endif
       enddo
   end do
...
151 format('MARKER_ELEMS= ', i10)
   ...
   do i=1,n_Names
       ...
       if (Names(i)=='FARFIELD') then 
           Nfarfield=Nfarfield+1
       endif
       if (Names(i)=='FLUID') then 
           Nfluid=Nfluid+1
       endif
   enddo
   Nsurface=N_names-Ninlet-Noutlet-Nfarfield-1
   !------------------------------------------------------------------------------------
   ...
   if (N_names>0) then
    ...
    do i=1,N_names
       if (Names(i) .ne. 'INLET' .and. Names(i) .ne. 'OUTLET' .and. Names(i) .ne. 'FLUID'.and. Names(i) .ne. 'FARFIELD') then
   ...
   write(IOinp,'(''% FarField boundary marker(s) (NONE = no marker) '')') 
   if (Nfarfield>0) then
    do i=1,MaxNames
       if (Names(i)=='FARFIELD') then
          write(IOinp,'( "MARKER_FAR=(",i4,")" )') i
       endif
    end do  
	write(*,*) Nfarfield,'Farfield.'
   else
    write(IOinp,'(''MARKER_FAR= NONE'')') 
	write(*,*) 'No farfield.'
   endif
   write(IOinp,'(''%'')') 
  ...
  !1001 format(1x,'MARKER_INLET=(',i10,'288.6, 102010.0, 1.0, 0.0, 0.0)')
   ...
   if (Nfluid>0) then
	 ...
	 write(*,*) 'Error: No fluid. One 2D physical surface must be FLUID.'
   endif
   write(IOinp,'(''%'')') 
